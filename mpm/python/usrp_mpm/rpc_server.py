...
class MPMServer(RPCServer):
    ...
    def __init__(self, state, default_args):
        ...
        self.log.debug("Launching RPC server with compat num %d.%d",
    ...
    def reclaim(self, token):
        ...
        if self._state.claim_status.value:
            ...
            if self._check_token_valid(token):
                self._state.lock.release()
                self.log.trace("reclaimed from: %s", self.client_host)
                self._reset_timer()
                return True
            self._state.lock.release()
            self.log.trace(
    ...
    def _timeout_event(self):
        ...
        if self._disable_timeouts:
            self.log.trace("Timeouts are disabled: Snoozing")
            self._reset_timer()
        else:
    ...
    def get_log_buf(self, token):
        ...
        self.log.debug("Returning %d log records.", len(log_records))
        return [
    ...
    def update_component(self, token, file_metadata_l, data_l):
        ...
        with self._timeout_disabler():
            ...
            try:
                self.log.debug("Reset after updating component? {}".format(reset_now))
                if reset_now:
                    self._reset_mgr()
                    self.log.debug("Reset the periph manager")
            except Exception as ex:
