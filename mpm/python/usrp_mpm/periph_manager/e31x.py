...
class e31x(ZynqComponents, PeriphManagerBase):
    ...
    def _init_gps_sensors(self):
        ...
        """
        self.log.debug("Initializing GPSd interface")
        ...
        for method_name in new_methods:
            try:
                ...
                self.log.debug("Adding %s sensor function", sensor_name)
    ...
    def _init_dboards(self, dboard_infos, override_dboard_pids, default_args):
        ...
        assert spi_nodes
        self.log.debug("Found spidev nodes: {0}".format(str(spi_nodes)))
        dboard_info.update({
            'spi_nodes': spi_nodes,
    ...
    def _read_mboard_eeprom(self):
        ...
        #eeprom_path = \
        #   get_eeprom_paths(self.mboard_eeprom_addr)
        eeprom_path = "/sys/class/Unknow/empty"
        if not eeprom_path:
            self.log.error("Could not identify EEPROM path for %s!",
                           self.mboard_eeprom_addr)
            return

        self.log.debug("MB EEPROM: Using path {}".format(eeprom_path))
        ...
        self.log.debug("Read %d bytes of EEPROM data.", len(self.eeprom_rawdata))
    ...
    def remove_idle_overlay(self):
        ...
        """
        idle_overlay = self.get_idle_dt_overlay(self.device_info)
        self.log.debug("Removing Idle overlay: {}".format(
            idle_overlay
        ))
    ...
    def tear_down(self):
        ...
        """
        self.log.debug("Tearing down E310 device...")
        self.dboards = []
        ...
        self._device_initialized = False
        active_overlays = self.list_active_overlays()
        self.log.debug("E310 has active device tree overlays: {}".format(
            active_overlays
        ))
        for overlay in active_overlays:
            dtoverlay.rm_overlay(overlay)
    ...
    def is_idle(self):
        ...
        if is_idle:
            self.log.debug("Found idle overlay: %s", idle_overlay)
        return is_idle
    ...
    def get_clock_sources(self):
        ...
        self.log.debug("Listing available clock sources...")
    ...
    def set_time_source(self, time_source):
        ...
        if time_source == self.get_time_source():
            self.log.debug("Nothing to do -- time source already set.")
            return
    ...
    def set_gpio_src(self, bank, src):
        ...
        self.log.debug("Updating GPIO source: master==0x{:02X} radio_src={:03X}"
    ...
    def get_ref_lock_sensor(self):
        ...
        """
        self.log.debug("Querying ref lock status.")
    ...
    def get_mb_temp_sensor(self):
        ...
        """
        self.log.debug("Reading temperature.")
        temp = '-1'
        raw_val = {}
